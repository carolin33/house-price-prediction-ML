...
# ============================================================
evaluate_and_update("C", [0.1, 1, 10, 100])
evaluate_and_update("kernel", ["linear", "rbf", "poly", "sigmoid"])
evaluate_and_update("gamma", ["scale", "auto", 0.01, 0.1])
evaluate_and_update("epsilon", [0.01, 0.1, 0.5, 1.0])

results_df = pd.DataFrame(all_results)
results_df.to_csv("wyniki_svm_regresja.csv", index=False)

print("\nFINALNE PARAMETRY SVM:")
...
metrics_df.to_csv("metryki_train_test_svm.csv", index=False)
...
for ax, param in zip(axes, ["C", "kernel", "gamma", "epsilon"]):
...
plt.tight_layout()
plt.savefig("svm_wykres_greedy.png", bbox_inches="tight")
plt.close()
...
plt.tight_layout()
plt.savefig("svm_wykres_train_test.png", bbox_inches="tight")
plt.close()

print("\nZapisano wyniki i wykresy dla SVM.")