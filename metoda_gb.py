...
def evaluate_parameter(param_name, values):
    ...
    for val in values:
        ...
        clf_pipe = Pipeline([
            ...
            ("model", GradientBoostingClassifier(**params_clf))
        ])

        reg_pipe = Pipeline([
            ("preprocess", preprocessor_reg),
            ("model", GradientBoostingRegressor(**params_reg))
        ])
...
# 7. EKSPERYMENTY
# ============================================================
evaluate_parameter("n_estimators", [50, 100, 200, 300])
evaluate_parameter("learning_rate", [0.01, 0.05, 0.1, 0.2])
evaluate_parameter("max_depth", [2, 3, 4, 5])
evaluate_parameter("subsample", [0.6, 0.8, 0.9, 1.0])
